...
impl Evaluator for ConsensusRoundEvaluator {
    ...
    async fn evaluate(&self, input: &Self::Input) -> Result<Vec<EvaluationResult>, Self::Error> {
        ...
        let previous_round = match self.get_consensus_round(&input.previous_target_metrics, "first")
        {
        ...
        let latest_round = match self.get_consensus_round(&input.latest_target_metrics, "second") {
            GetMetricResult::Present(metric) => Some(metric),
            GetMetricResult::Missing(evaluation_result) => {
                evaluation_results.push(evaluation_result);
                None
