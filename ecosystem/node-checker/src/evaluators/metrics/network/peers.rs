...
impl NetworkPeersWithinToleranceEvaluator {
    ...
    fn build_evaluation(
        ...
        let name = connection_type.get_name();
        let particle = connection_type.get_particle();
        if target_connections < baseline_connections - tolerance
            || target_connections > baseline_connections + tolerance
        {
            self.build_evaluation_result(
                format!("The number of {} connections {} the target node is too different compared to the baseline", name, particle),
                50,
                format!("The target node has {} {} connections {} other nodes, which is too different compared to the baseline: {} (tolerance: {})", target_connections, name, particle, baseline_connections, tolerance),
            )
        } else {
            self.build_evaluation_result(
                format!("The number of {} connections {} the target node looks good", name, particle),
                100,
                format!("The target node has {} {} connections with other nodes, which is within tolerance ({}) of the value from the baseline: {}", target_connections, name, tolerance, baseline_connections),
            )
        }
    }
}
